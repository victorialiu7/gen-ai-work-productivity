...
# # Create cleaned mini-dataframes
dfs <- lapply(result, function(cols) {
  g %>%
    ...
    relocate(prolific_pid, .before = everything())  # move prolific_pid to first
})
...
# Step 2: Reduce (full join) all the renamed dataframes
...
#   mutate(across(everything(), ~ ifelse(sapply(.x, is.null), 0, .x)))
...
# # # Average every cell in "used" column
# cleaned_df <- cleaned_df %>%
#   mutate(used = sapply(used, function(x) mean(x, na.rm = TRUE)))
